...
print('9. Какое максимальное число часов человек работает в неделю (признак hours-per-week)? Сколько людей работают такое '
      'количество часов и каков среди них процент зарабатывающих много?')
...
print('10. Посчитайте среднее время работы (hours-per-week) зарабатывающих мало и много (salary)'
      ' для каждой страны (native-country).')
...
print('11. группируйте людей по возрастным группам young, adult, retiree, где:'
      'young соответствует 16-35 лет'
      'adult - 35-70 лет '
      'retiree - 70-100 лет'
      'Проставьте название соответсвтуещей группы для каждого человека в новой колонке AgeGroup')
...
print('12. Определите количество зарабатывающих >50K в каждой из возрастных групп (колонка AgeGroup), '
      'а также выведите название возрастной группы, в которой чаще зарабатывают больше 50К (>50K)')
...
print('13. Сгруппируйте людей по типу занятости (колонка occupation) и определите количество людей в каждой группе. '
      'После чего напишите функцию фильтрации filter_func, которая будет возвращать только те группы, '
      'в которых средний возраст (колонка age) не больше 40 и в которых все работники отрабатывают '
      'более 5 часов в неделю (колонка hours-per-week)')
...
def filter_func(df_func):
    grouping_df = df_func.groupby('occupation', as_index=False).agg({'age': 'mean', 'hours_per_week': 'min'}).rename(
        columns={'age': 'avg_age', 'hours_per_week': 'min_hours_per_week'})
    print(grouping_df)
